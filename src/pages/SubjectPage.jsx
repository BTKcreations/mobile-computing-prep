...
function SubjectPage() {
    ...
    // State
    const [viewMode, setViewMode] = useState(paramViewMode || 'study')
    const [unitView, setUnitView] = useState({}) // 'notes' or 'topics'
    const [selectedTopic, setSelectedTopic] = useState(null)
    const [activeMindMap, setActiveMindMap] = useState(null)
    const [zoomLevel, setZoomLevel] = useState(1)
    const [quizMode, setQuizMode] = useState(null) // 'I', 'II', ... or 'FINAL'
    ...
    useEffect(() => {
        if (subjectData) {
            const saved = localStorage.getItem(subjectData.progressKey)
            if (saved) setProgress(JSON.parse(saved))
            else setProgress({})

            // Initialize flashcards
            if (subjectData.flashcards) {
                setShuffledCards([...subjectData.flashcards].sort(() => 0.5 - Math.random()))
            }
        }
    }, [subjectData?.id])

    // Handle Topic Deep Linking
    useEffect(() => {
        if (topicId && subjectData) {
            // Find topic in syllabus
            let foundTopic = null
            for (const unit of subjectData.syllabus) {
                const topic = unit.topics.find(t => t.id === topicId || t.title.toLowerCase().replace(/\s+/g, '-') === topicId)
                if (topic) {
                    foundTopic = topic
                    break
                }
    ...
    if (!subjectData) return <div className="container">Subject not found</div>

    // Helper Functions
    const saveProgress = (unit, score, total) => {
        const newProgress = { ...progress, [unit]: { score, total } }
        setProgress(newProgress)
        localStorage.setItem(subjectData.progressKey, JSON.stringify(newProgress))
    }

    const toggleSection = (unit, section) => {
        setUnitView(prev => ({
            ...prev,
            [unit]: prev[unit] === section ? null : section
        }))
    }

    const openTopic = (topic) => {
        // Navigate to topic URL
        // Assuming topic has an ID, if not use title slug
        const tId = topic.id || topic.title.toLowerCase().replace(/\s+/g, '-')
        navigate(`/subject/${subjectId}/topic/${tId}`)
    }
    ...
    const handleViewModeChange = (mode) => {
        setViewMode(mode)
        navigate(`/subject/${subjectId}/${mode}`)
    }
        ...
        <div className="container">
            <header>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    <button className="btn" onClick={() => navigate('/')}>&larr; Switch Subject</button>
                ...
                <div className="breadcrumbs" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', color: '#64748b', margin: '1rem 0', justifyContent: 'center' }}>
                    ...
                    {topicId && (
                        <>
                            <span>&gt;</span>
                            <span>{selectedTopic ? selectedTopic.title : topicId}</span>
                        </>
                    )}
                </div>

                <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '1rem', flexWrap: 'wrap' }}>
                    <button className={`btn ${viewMode === 'study' ? 'btn-primary' : ''}`} onClick={() => handleViewModeChange('study')}>Study Mode</button>
                    <button className={`btn ${viewMode === 'paper' ? 'btn-primary' : ''}`} onClick={() => handleViewModeChange('paper')}>Model Papers</button>
                    <button className={`btn ${viewMode === 'tips' ? 'btn-primary' : ''}`} onClick={() => handleViewModeChange('tips')}>Exam Tips</button>
                    <button className={`btn ${viewMode === 'flashcards' ? 'btn-primary' : ''}`} onClick={() => handleViewModeChange('flashcards')}>Flashcards</button>
                </div>
